...
async def download_images(url: list) -> list[Path]:

    # with requests.Session() as session:
    # downloadsingleimage = [asyncio.create_task(asyncio.to_thread(download_single_image(), session, url, img_num)) for i in range(1,13)]
    # img_path = await asyncio.gather(*downloadsingleimage)

    async with httpx.AsyncClient() as client:
        async with asyncio.TaskGroup() as tg:
            tasks = [tg.create_task(download_single_image(client, url, img_num)) for img_num, url in enumerate(url,start=1)]

        img_path = [task.result() for task in tasks]
...
async def process_images(org_paths: list[Path]) -> list[Path]:
    loop = asyncio.get_running_loop()
     
    with ProcessPoolExecutor() as executor:
        tasks = [loop.run_in_executor(executor, process_single_image, org_path) for org_path in org_paths]
